...
fn Simulation(
	...
	for (var j: i32 = -I; j <= I; j = j + 1) {
        ...
        var data: GridSlotEncoded = buffer_a.pixels[get_index(vec2<i32>(tpos))];
...
fn update(@builtin(global_invocation_id) invocation_id: vec3<u32>) {
    ...
    // G2R is a private global variable, and it's used in border()
    R2G = R / vec2<f32>(uni.grid_size.xy);

	time = uni.iTime;
	Mouse = uni.iMouse;

    var data: GridSlotEncoded = buffer_a.pixels[get_index(vec2<i32>(pos))];
	...
	let U: GridSlot = saveParticles(P, P, pos);

    buffer_b.pixels[get_index(location)] = U;
