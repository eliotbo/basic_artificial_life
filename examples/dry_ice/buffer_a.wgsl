...
fn fbm(p: vec3<f32>, octaveNum: i32) -> vec2<f32> {
...
fn sampleMinusGradient(coord: vec2<f32>) -> vec3<f32> {
	var veld: vec3<f32> = textureLoad_CONVERT_TO_i32(BUFFER_iChannel1, vec2<i32>(coord / uni.iResolution.xy)).xyz;
	let left: f32 = textureLoad_CONVERT_TO_i32(BUFFER_iChannel0, vec2<i32>((coord + vec2<f32>(-1., 0.)) / uni.iResolution.xy)).x;
	let right: f32 = textureLoad_CONVERT_TO_i32(BUFFER_iChannel0, vec2<i32>((coord + vec2<f32>(1., 0.)) / uni.iResolution.xy)).x;
	let bottom: f32 = textureLoad_CONVERT_TO_i32(BUFFER_iChannel0, vec2<i32>((coord + vec2<f32>(0., -1.)) / uni.iResolution.xy)).x;
	let top: f32 = textureLoad_CONVERT_TO_i32(BUFFER_iChannel0, vec2<i32>((coord + vec2<f32>(0., 1.)) / uni.iResolution.xy)).x;
...
fn update([[builtin(global_invocation_id)]] invocation_id: vec3<u32>) {
	...
	for (var i: i32 = 0; i < nbSphere; i = i + 1) {
		let p: vec2<f32> = spherePosition(i, uni.iFrame);
		let dist: f32 = distance(uv, p);
		if (dist < influenceRadius) {
			let op: vec2<f32> = spherePosition(i, uni.iFrame + 1);
