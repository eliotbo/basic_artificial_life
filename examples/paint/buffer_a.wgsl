...
//             bufferStore(buffer_a, location, hue(4.0 / 8.0));
//         } else {
//             // set brush color to black
//             let black = vec4<f32>(0.0, 0.0, 0.0, 1.0);
//             bufferStore(buffer_a, location, black);
//         }
//     # endif
// }
...
fn update([[builtin(global_invocation_id)]] invocation_id: vec3<u32>) {
    ...
    # ifdef INIT
    if (location.x == 0 && location.y == 0) {
        // bufferStore( location, hue(4.0 / 8.0));
         buffer_a.pixels[get_index(location)] =  hue(4.0 / 8.0);
    } else {
        // set brush color to black
        let black = vec4<f32>(0.0, 0.0, 0.0, 1.0);
        // bufferStore( location, black);
        buffer_a.pixels[get_index(location)] =  black;
    }
    # endif
    // }
    ...
    if (U.x < 0.1) {
        ...
        O.w = 1.;
        // bufferStore( location, O);
        buffer_a.pixels[get_index(location)] = O;
        return;
    }

    // let brush_color = vec4<f32>(1., 0., 0., 1.);
    // let brush_color = bufferLoad( vec2<i32>(0, 0));
    let brush_color = buffer_a.pixels[get_index(vec2<i32>(0, 0))]; 
