fn t(v: vec2<f32>) -> vec4<f32> {
	return  buffer_a.pixels[get_index( vec2<i32>(v ))];
    // textureLoad(buffer_a, vec2<i32>(v ));
} 
...
fn update([[builtin(global_invocation_id)]] invocation_id: vec3<u32>) {
    ...
    textureStore(texture, y_inverted_location, toLinear(C));

    // me.a = 1.0;
    // textureStore(texture, y_inverted_location, toLinear(me * 10.));

    // textureStore(texture, y_inverted_location, buffer_a.pixels[get_index( location)] );
} 
