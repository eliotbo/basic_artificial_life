...
fn update([[builtin(global_invocation_id)]] invocation_id: vec3<u32>) {
    ...
    let data: vec4<f32> = buffer_a.pixels[get_index(location)];
    // textureLoad(buffer_a, location);
    ...
    for (var i: i32 = -1; i <= 1; i = i + 1) {
        for (var j: i32 = -1; j <= 1; j = j + 1) {
            ...
            let data: vec4<f32> =  buffer_a.pixels[get_index(location + ij)];
            // textureLoad(buffer_a, location + ij);
    ...
    buffer_c.pixels[get_index(location)] = rho;
} 
